...
- 지도(`/map`): 카카오맵 SDK로 지도 표시 (로그인 불필요)
- 시세(`/stock`): 삼성전자·SK하이닉스·카카오·두산에너빌리티 실시간 시세, 10초 자동 갱신 + 수동 새로고침 + 카운트다운 표시 (로그인 불필요)
- 번역(`/translate`): MyMemory API로 텍스트 번역, 언어 선택/스왑 지원 (로그인 불필요)
- 스포츠(`/news`): 연합뉴스·SBS·동아일보 스포츠 RSS 3개를 합쳐 최신순으로 표시, 제목 클릭 시 새 탭에서 원문으로 이동 (로그인 불필요)
